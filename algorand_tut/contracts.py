...
def build_distributed_treasury_app(
    ...
    app_id = tl.Global.current_application_id()
    ...
    is_opted_in = tl.App.optedIn(tl.Txn.sender(), app_id)
    ...
    is_valid_nomination = tl.And(
        ...
        tl.If(locals.maybe("last_nomination_ts").hasValue())
        .Then(timestamp - locals.get("last_nomination_ts") > cooldown)
        .Else(one),
    )
    ...
    is_valid_withdrawl = tl.And(
        ...
        tl.Le(
            withdrawl_amount,
            tl.If(globals.get("term_budget") <= globals.get("votes_for"))
            .Then(globals.get("term_budget") - globals.get("term_used"))
            .Else(globals.get("votes_for") - globals.get("term_used")),
        ),
    )
    ...
    remove_vote = tl.Seq(
        tl.If(
            ...
            tl.If(locals.maybe("last_vote_ts").hasValue())
            .Then(timestamp - locals.get("last_vote_ts") <= voting_duration)
            .Else(zero)
        ).Then(
    ...
    invokations["nominate"] = tl.Seq(
        tl.If(tl.Not(is_opted_in)).Then(tl.Return(zero)),
        tl.Seq(globals.load_maybe(), locals.load_maybe()),
        tl.If(tl.Not(is_valid_nomination)).Then(tl.Return(zero)),
    ...
    invokations["vote_for"] = tl.Seq(
        tl.If(tl.Not(is_opted_in)).Then(tl.Return(zero)),
        tl.Seq(globals.load_maybe(), locals.load_maybe()),
        tl.If(tl.Not(is_state_voting)).Then(tl.Return(zero)),
        # clear previous vote
        remove_vote,
    ...
    invokations["withdraw_funds"] = tl.Seq(
        tl.If(tl.Not(is_opted_in)).Then(tl.Return(zero)),
        tl.Seq(globals.load_maybe(), locals.load_maybe()),
        tl.If(tl.Not(is_valid_withdrawl)).Then(tl.Return(zero)),
        ...
        tl.InnerTxnBuilder.SetField(tl.TxnField.receiver, globals.get("nominee")),
